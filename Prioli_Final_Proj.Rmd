...
To test for a difference in infant mortality rate by human development category, Analysis #5 employed the permutation F-test and ANOVA as the nonparametric and parametric tests respectively.  Both tests assume samples are independent and identically distributed, and ANOVA additionally assumes a normal distribution.  The permutation F-test was performed using `jmuOutlier::perm.f.test()` using 1,000 simulations, whereas `aov()` was used for ANOVA testing.

<!-- OMITTING - Analysis #6 -->

<!-- Analysis #6 sought to determine whether there is a difference in median between $\frac{1}{100}$`SPI` and `genderequality_index`.  Dividing `SPI` by 100 ensures it shares the same 0:1 scale as `genderequality_index`.  After this rescaling, the two variables appear to have similar medians on descriptive analysis.  The Wilcoxon Rank-Sum test assumes all observations are independent and come from the same distribution under $H_{0}$, whereas the two-sample *t*-test assumes independent samples and normality of the sampling distribution.  A two-sided alternative was used for each test.  Wilcoxon Rank-Sum testing was carried out using the `wilcox.test()` function, and the two-sample *t*-test via `t.test()`. -->
...
<!-- analysis6_nonpara <- wilcox.test((alldata$SPI / 100), alldata$genderequality_index, -->
<!--                                  alternative = "two.sided", exact = TRUE, -->
<!--                                  conf.int = TRUE, conf.level = 0.95) -->
<!-- analysis6_nonpara -->
<!-- analysis6_para <- t.test((alldata$SPI / 100), alldata$genderequality_index) -->
<!-- analysis6_para -->
<!-- ``` -->
