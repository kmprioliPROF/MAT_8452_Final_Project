...
tbl2_vars <- c("HDIindex",
               ...
               "happiness",
               "HDI_cat, infantmort")
tbl2_null <- c("(No formal hypothesis test) Nonparametric 95% confidence interval for the mean",
...
tbl2_alt <- c("Parametric 95% CI for the mean",
...
In Analysis #1, bootstrapping and the one-sample *t*-test were employed to find nonparametric and parametric 95% confidence intervals (CIs) for the population mean.  The widths of these intervals were compared, and assumptions required for the *t*-test were assessed.  The bootstrapping CI for the mean was found using 1000 sampling iterations with replacement via `sample()`, and the CI for the mean was found via `t.test()`.
...
In Analysis 1, the bootstrapping approach yielded a 95% CI for the mean of (`r analysis1_alt_nonpara[[1]], analysis1_alt_nonpara[[2]]`), having width `r analysis1_alt_nonpara[[3]]`.  The parametric approach via `t.test()` yielded a 95% CI given by (`r analysis1_alt_para[[1]], analysis1_alt_para[[2]]`), having width `r analysis1_alt_para[[3]]`.  The nonparametric method yielded the smaller confidence interval, having `r patse0(round((analysis1_alt_nonpara[[3]] / analysis1_alt_para[[3]]) * 100, digits = 2), "%"` the width of the CI obtained through the parametric method.  The one-sample *t*-test assumes that the sample is random with independent draws, and that the data are continuous and normally distributed.  Independence and continous data are met, but per Fig. 1, normality is violated, so the parametric approach is not appropriate.  The nonparametric approach dominates.
