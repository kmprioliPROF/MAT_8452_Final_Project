...
Because the `HDIindex` distribution is very non-normal, yet its median and mean appear quite similar, Analysis #1 employed a two-sided one-sample test to find a 95% confidence interval (CI) for the population median.  The one-sample sign test assumes that the sample is random with independent draws, and the data are continuous.  Its parametric equivalent, the one-sample *t*-test, used to find the 95% CI for the population mean, shares these assumptions and also requires normality of the sampling distribution.  The CI for the median was found via `BSDA::SIGN.test()` and the CI for the mean was found via `t.test()`.
...
For Analysis 1, the 95% confidence interval for the population median was (`r round(analysis1_nonpara$conf.int[1], 4)`, `r round(analysis1_nonpara$conf.int[2], 4)`), and the CI for the population mean was (`r round(analysis1_para$conf.int[1], 4)`, `r round(analysis1_para$conf.int[2], 4)`). -->

<!-- ### *Analysis #1 alt* -->

```{r analysis1_alt, include = FALSE, echo = FALSE}
#analysis1_alt_nonpara
...
analysis1_alt_nonpara <- meanCI_BS
analysis1_alt_nonpara


analysis1_alt_para <- t.test(alldata$HDIindex,
                         alternative = "two.sided",
                         conf.level = 0.95)
...
meanCI_ttest[1, 1] <- round(analysis1_alt_para$conf.int[1], digits = 4)
meanCI_ttest[1, 2] <- round(analysis1_alt_para$conf.int[2], digits = 4)
meanCI_ttest$width = meanCI_ttest$CI_high - meanCI_ttest$CI_low

analysis1_alt_para <- meanCI_ttest
analysis1_alt_para
```

In Analysis 1, the bootstrapping approach yielded a 95% CI for the mean of (`r analysis1_alt_nonpara[[1]], analysis1_alt_nonpara[[2]]`), having width `r analysis1_alt_nonpara[[3]]`.  The parametric approach via `t.test()` yielded a 95% CI given by (`r analysis1_alt_para[[1]], analysis1_alt_para[[2]]`), having width `r analysis1_alt_para[[3]]`.  The nonparametric method yielded the smaller confidence interval, having `r patse0(round((analysis1_alt_nonpara[[3]] / analysis1_alt_para[[3]]) * 100, digits = 2), "%"` the width of the CI obtained through the parametric method, so the nonparametric method dominates.
