...
<!-- Notes
  ...
  * Add to Results:
    ...
    * Plot `happiness` overlaid with pnorm curve and/or a 95% confidence band
-->
...
fulldata <- read_csv("data/MAT_8790_alldata_2018-12-02.csv")
fulldata <- fulldata %>% 
  mutate(country = factor(country)) %>% 
  mutate(HDI_cat = factor(HDI_cat, levels = c("Low", "Medium", "High", "Very High")))
...
analysis5_plot <- ggplot(data = alldata, aes(x = HDI_cat, y = infantmort)) +
  ...
  ggtitle("Figure 9.  Infant Mortality Rate by HDI Category")
analysis5_plot
```
